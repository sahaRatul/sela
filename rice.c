...
unsigned int rice_encode_block(short param,const unsigned short *input,int size,unsigned short *encoded)
{
	...
	rice_encode_context *ctx = (rice_encode_context *)malloc(sizeof(rice_encode_context));
	ctx->buffer = 0;
	ctx->filled = 0;
	ctx->bits = 0;
...
unsigned int rice_decode_block(short param,const unsigned short *encoded,int size,unsigned short *decoded)
{
	int i = 0,j = 0,q,k = 0;
	short tmp;
	rice_decode_context *ctx = (rice_decode_context *)malloc(sizeof(rice_decode_context));
	ctx->filled = 0;
